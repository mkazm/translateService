...
public class RegisterController {
    ...
    public String addUser(HttpServletRequest request, UserModel user, Model model, RedirectAttributes ra) {
	
	
	boolean emailReserved = getMana().getUserDao().checkEmailIfReserved(user.getEmail());
	if(emailReserved == true){
	    model.addAttribute("error", Dictionary.REGISTER_ERROR_EMAIL_USED);
	    return "/register/register";
	}
	
	UserModel userCheck = getMana().getUserDao().findByUsername(user.getUsername());
	String hash;
...
//	Tools.sendMail(userCheck.getId(), hash, userCheck.getEmail(), Tools.getRootUrl(request));
    ...
    public String confirmPassword(@PathVariable long id, @PathVariable String conf , Model model, RedirectAttributes ra){
